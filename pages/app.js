...
import { Heading, Flex, Button, Text, Box, Divider, Grid, FormControl, FormLabel, NumberInput, NumberInputField, NumberDecrementStepper,NumberIncrementStepper, NumberInputStepper } from "@chakra-ui/react";
...
import FormBNB from "../components/FormBNB";
...
export default function Main() {
  ...
  console.log(regex.test(tweetUrl)) */
    if (tweetUrl.length < 1) return;
    setLoading(true);
    const response = await getTweet(tweetUrl.split('?')[0].split("/").at(-1));
    setLoading(false);
  ...
  const handlePrint = async (print) => {
    setBorderPrint(true);
    domtoimage
      .toPng(print, { width: 600, height: 600 })
        ...
        <>
          {tweet && (
            <Flex
              ...
              direction={{ base: "column", md: "row" }}
            >
              <Flex maxW={{ base: "100%", md: "250px"}} w="100%" direction="column" mr={8}>
                <Text
                ...
                <Grid
                  ...
                  gridTemplateColumns={{
                    base: "repeat(auto-fill, minmax(72px, 1fr))",
                    md: "repeat(auto-fill, 100px)",
                  }}
                  autoFlow={{ base: "column", md: "row" }}
                  autoColumns={{
                    base: "minmax(72px, 1fr)",
                    md: "minmax(100px, 1fr)",
                  }}
                ...
                >
                  {designs.map((item, i) => (
                    <DesignItem
                      ...
                      designActive={designActive === item.id}
                    />
                  ))}
                </Grid>
              </Flex>
