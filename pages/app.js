...
/*     const regex = /((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((?:\/[\+~%\/.\w-_]*)?\??(?:[-\+=&;%@.\w_]*)#?(?:[\w]*))?)/
  ...
  return (
    <>
      ...
      <Flex m="0 auto" maxW="1200px" w="100%" align="center" justify="space-between" py={7} px={4}>
        <Brand />
        <InputSearch tweetUrl={tweetUrl} setTweetUrl={setTweetUrl} handleStyle={handleStyle} />

        <Button variant="ghost" onClick={toggleColorMode} borderRadius="full" p={3}>
          <VscColorMode />
        </Button>
      </Flex>
      ...
      { loading ? (
          ...
          <>
            { tweet &&
                <Flex maxW="1200px" m="0 auto"  p={4}>
                    <Flex w="30%" direction="column">
                      <Text letterSpacing="-0.25px">Choose a design</Text>

                      <Wrap w="100%" h="100%" maxH="300px" mt={4} spacing={3}>
                        {
                          designs.map( (item, i) => (
                            <DesignItem item={item} key={i} handleDesign={handleDesign} designActive={designActive === i}/>
                          ))
                        } 
                      </Wrap>
                    ...
                    <Box w="100%" >
                      <Box ref={print}>
                        <MainTweet selectedDesign={selectedDesign} tweet={tweet} borderPrint={borderPrint}/>
                      </Box>
